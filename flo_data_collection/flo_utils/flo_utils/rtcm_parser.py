...
class RTCMParser:
    ...
    def __init__(self, 
        ...
        # Bit of a strange pattern here, 
        # but save the log functions so we can be agnostic of ROS
        self._logerr = logerr
    ...
    def parse(self, buffer):
        ...
        while index < len(combined_buffer):
            ...
            if combined_buffer[index] == _RTCM_3_2_PREAMBLE:
                ...
                if len(combined_buffer) <= index + 2:
                    self._logdebug(
                        f'Found beginning of RTCM packet at {index}, '+\
                          'but there is not enough data in the buffer to find the message length')
                ...
                message_length = (
                    combined_buffer[index + 1] << 8 | combined_buffer[index + 2]) & 0x03FF
                if index + message_length + 6 <= len(combined_buffer):
                    # Grab the packet from the buffer, and verify 
                    # that it is valid by comparing checksums
                    ...
                    if expected_checksum == actual_checksum:
                        self._logdebug(
                            f'Found valid packet at {index} with length {message_length}')
                        ...
                        self._logwarn(
                            f'Expected Checksum: 0x{expected_checksum:X}')
                        self._logwarn(
                            f'Actual Checksum:   0x{actual_checksum:X}')
                else:
                    self._logdebug(
                        f'Found beginning of RTCM packet at {index}, '+\
                          'but there is not enough data in the buffer to extract it, caching')
                    self._caching_data = True
        ...
        if self._caching_data:
            ...
            if len(self._buffer) > _MAX_BUFFER_SIZE:
                self._logwarn(
                    f"Too much data buffered, trimming to {_MAX_BUFFER_SIZE} bytes.")
    ...
    def _checksum(self, packet):
        crc = 0
        for byte in packet:
            crc = ((crc << 8) & 0xFFFFFF) ^ _RTCM_CRC_LOOKUP[(
                crc >> 16) ^ byte]
        return crc
